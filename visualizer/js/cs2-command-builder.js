(function (App) {
  ...
  function buildFullBundleCommand(state, bundleMeta) {
    ...
    return [
      ...
      "}",
      "",
      "if (-not (Test-Path '.\\tools\\build_complete_bundle.py')) { throw 'Lance ce script depuis la racine du depot cs2-realmap-generator.' }",
      "",
      ...
      "",
      "& " + quoteArg(pythonExe) + " " + quoteArg(".\\tools\\build_complete_bundle.py") + " `",
      "  --city $city `",
      "  --country $country `",
      ...
      "",
      "Assert-PythonSuccess " + quoteArg("Bundle complet : extraction, PNG, validations et synchronisation"),
      "",
      "tree $bundle /F"
    ].join("\n");
  }
