(function (App) {
  ...
  function buildFullBundleCommand(state, bundleMeta) {
    ...
    return [
      "$ErrorActionPreference = " + quoteArg("Stop"),
      "",
      "function Assert-PythonSuccess([string]$step) {",
      "  if ($LASTEXITCODE -ne 0) {",
      "    throw " + quoteArg("$step a échoué avec le code $LASTEXITCODE."),
      "  }",
      "}",
      "",
      ...
      "  --split-layers",
      "",
      "Assert-PythonSuccess " + quoteArg("Extraction GeoJSON"),
      "",
      "$railwaysGeoJson = Join-Path $geoDir " + quoteArg("geojson\\railways.geojson"),
      "if (-not (Test-Path -LiteralPath $railwaysGeoJson -PathType Leaf)) {",
      "  throw " + quoteArg("railways.geojson n'a pas été généré."),
      "}",
      "",
      ...
      "",
      "Assert-PythonSuccess " + quoteArg("Génération des PNG"),
      "",
      ...
      "  --check-existing",
      "",
      "Assert-PythonSuccess " + quoteArg("Écriture du manifeste"),
      "",
      ...
      "  --pixels " + quoteArg(HEIGHTMAP_PIXELS),
      "",
      "Assert-PythonSuccess " + quoteArg("Validation des PNG"),
      "",
      ...
      "$timelineBundle = Join-Path $timelineBundles $bundleId",
      "",
      "New-Item -ItemType Directory -Force $timelineBundles | Out-Null",
      "New-Item -ItemType Directory -Force $timelineBundle | Out-Null",
      "Copy-Item " + quoteArg(".\\exports\\bundles\\bundle_index.json") + " " + quoteArg("$timelineBundles\\bundle_index.json") + " -Force",
      "Get-ChildItem -LiteralPath $bundle -Force | Copy-Item -Destination $timelineBundle -Recurse -Force",
